...
if __name__ == "__main__":
    ...
    for iter in range(START_ITER, NUM_REPETITIONS):
        ...
        df_results = pd.read_csv(
            os.path.join(
                OUTPUT_DIR, "iter_{}".format(iter), fname_params["molecules_fname"]
            )
        )
        ...
        df = pd.DataFrame(
            {
                "best_smiles": best_molecules_list,
                "reward": max_rewards_list,
                "gap_reward": max_gap_rewards_list,
                "sim_reward": max_sim_rewards_list,
            }
        )
        ...
        if os.path.exists(os.path.join(OUTPUT_DIR, fname_params["fingerprints_fname"])):
            fingerprints = np.load(
                os.path.join(OUTPUT_DIR, fname_params["fingerprints_fname"])
            )
        else:
            fingerprints = np.empty((0, 2048), dtype=int)
        fingerprints = np.vstack((fingerprints, fp))
        np.save(
            os.path.join(OUTPUT_DIR, fname_params["fingerprints_fname"]), fingerprints
        )
