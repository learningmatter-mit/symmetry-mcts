...
def get_normalized_rewards(gap_reward, sim_reward, normalization_params):
    std_gap_reward = (gap_reward - normalization_params['bandgap']['mean']) / normalization_params['bandgap']['std_dev']
    std_sim_reward = (sim_reward - normalization_params['similarity']['mean']) / normalization_params['similarity']['std_dev']
...
def get_total_reward(gap_reward, sim_reward, train_params, reduction='sum'):
    if reduction == 'sum':
        return -1 * (train_params['sum_weights']['bandgap'] * gap_reward + train_params['sum_weights']['similarity'] * sim_reward)
    elif reduction == 'product':
